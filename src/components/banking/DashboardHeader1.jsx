...
export function DashboardHeader({
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://snapster.techykarthikbms.com/api/notification/${teller_id}/`
      );
  ...
  /* ---------------- Load notifications on mount (so badge appears immediately) ---------------- */
  useEffect(() => {
    fetchNotifications();
  }, [teller_id]); // re-fetch if teller_id changes

  /* ---------------- (Optional) Refresh when dropdown opens to get latest list ---------------- */
  useEffect(() => {
  ...
  /* ---------------- ❌ REMOVED the broken "Mark Read" useEffect ---------------- */
  ...
  //   }
  // }, [isNotificationOpen]);
  // That caused the badge to disappear immediately on open. Now removed.

  /* ---------------- Close dropdowns when clicking outside ---------------- */
  useEffect(() => {
    function handleClickOutside(event) {
      // Check if click is inside notification dropdown or its button
      const isInsideNotification =
        notificationDropdownRef.current?.contains(event.target) ||
        notificationButtonRef.current?.contains(event.target);

      // Check if click is inside account dropdown or its button
  ...
  return (
    <header ref={headerRef} className="flex items-center justify-between px-8 py-5 navy-gradient">
